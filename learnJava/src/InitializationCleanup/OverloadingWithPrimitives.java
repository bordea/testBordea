...
import week2.PrimitiveOverloading;
...
public class OverloadingWithPrimitives {
    ...
    void testConstVal() {                //pentru o constanta compilatorul cauta integer prima data
        System.out.print("5: ");
        f1(5);f2(5);f3(5);f4(5);f5(5);f6(5);f7(5); System.out.println();
    }

    void testChar() {                   //if char doesn;t find an exact match then it looks for int too, it' being promoted to integer
        char x = 'x';
        System.out.print("char: ");
        f1(x);f2(x);f3(x);f4(x);f5(x);f6(x);f7(x); System.out.println();
    }

    void testByte() {                   //it picks up the next type up available (ie short)
        byte x = 0;
    ...
    public static void main(String[] args) {
        OverloadingWithPrimitives p = new OverloadingWithPrimitives();
        p.testConstVal();
        p.testChar();
        p.testByte();
        p.testShort();
